# -*- coding: utf-8 -*-
"""
Created on Mon Jan 08 11:09:26 2025
...
# Constants
GUI_VERSION = 'MTP Alchemist A1 Ver0.0.25'
GUI_SIZE = '1920x900'
GUI_THEME = 'clam'  # e.g., 'winnative', 'clam', 'alt', 'default', 'classic', 'vista', 'xpnative'

SOURCE_EXCEL = r"Sirius_OTP_Fusemap_A1.xlsx"
...
class ExcelController:
    ...
    def _create_copy(self):
        ...
        # print(os.path.dirname(self.original_path))
        copy_path = os.path.join(os.path.dirname(self.original_path), f"Sirius_OTP_Fusemap_A1_copy_{timestamp}.xlsm")
        # print(copy_path)
    ...
    def _load_trac_map_tab(self):
        ...
        # Rename specific columns for easier handling
        df.rename(columns={
            df.columns[0]: 'MTP address',
            df.columns[2]: 'Field width',
            df.columns[4]: 'Record Field',
        ...
        # Sort the DataFrame by the 'MTP address' column
        df.sort_values(by='MTP address', inplace=True)
...
class ExcelGUI:
    ...
    def helpme(self):
        messagebox.showerror('HELP', 'Contact Caesal Cheng immediately')
    ...
    def _populate_tab_tree(self, tab_name, dataframe):
        ...
        for address, group in grouped_data:
            ...
            # group['Value'] = group['Value'].astype('object')
            # print(group['Value'])
            vals = group['Value'].dropna().unique().astype(str)
            value = ', '.join(vals) if len(vals) > 0 else ""
    
            # For CONFIGURATION tab, Byte is always 1.
            # For DUSR and CALIBRATION tabs, Byte value from 'Field width'
            if tab_name == "CONFIGURATION":
                Byte = 1
            else:
            ...
            # Format: (Register Name, MTP Address, RAM Address, Byte, Value)
