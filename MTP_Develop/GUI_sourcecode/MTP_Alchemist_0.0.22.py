...
# Constants
GUI_VERSION = 'MTP Alchemist Ver0.0.22'
GUI_SIZE = '1920x900'
...
class ExcelController:
    ...
    def calculate_crc_for_conf_range(self):
        ...
        crc_data = CRC_CONF_HEADER + values
        if not crc_data:
    ...
    def generate_binary_file_mtpfull(self, output_path):
        """Generate a binary file from the current values."""
        trac_hex_values = ['53', '31', '32', '33', '34', '35', '01', '01', '01', '48', '65', 'B3']
        acbi_hex_values = ['00', '00', '00', '00']
        ...
        # print(hex_values)
        binary_data = binascii.unhexlify(''.join(first_half_hex_values))
        with open(output_path, 'wb') as f:
            f.write(binary_data)
        ...
        if bbox:
            ...
            def save_value(*args):
                ...
                if self.controller:
                    ...
                    if tab_name == "CONFIGURATION" or "DUSR" or "CALIBRATION":
                        self.populate_matrix_tree()
                entry.destroy()
