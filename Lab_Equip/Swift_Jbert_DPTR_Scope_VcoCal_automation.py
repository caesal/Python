...
dir_pwr = {1.2: '1p2', 1.14: '1p14', 1.26: '1p26'}
...
def ModeSelect(CHIP, mode):
    if CHIP == 'PARADE':
        if mode == "ReTimerMode_noSSC":
            pwr.set_outputOFF()
            pwr.set_VoltageP6V(3.3)  # Force CONFIG1 GPIO to HIGH
            time.sleep(1)
        ...
        if mode == "ReDriverMode":
            ...
            pwr.set_outputOFF()
            pwr.set_VoltageP6V(3.3)  # Force CONFIG1/4 GPIO to HIGH
            time.sleep(1)
            pwr.set_outputON()
            time.sleep(1)
            pwr.get_CurrP25V()
            bert.set_SSC()
            aardvark.execute_batch_file(r"C:\Caesal\Bootup_Cali\A0_RT_0x32_I2C_SEQ_PIN_AUTO.xml")
            time.sleep(1)
        else:
            print("#################  WRONG MODE  #################")
    else:
...
def JBERT_Rate_Pattern(LR, LC, PT=0):
    if LR == 0x14:
        if LC == 0x2:
            ...
            bert.restart_All()
            bert.break_Sequence(1)
            bert.break_Sequence(1)  # CP2520 Pattern
    elif LR == 0xA:
        if LC == 0x2:
            ...
            bert.break_Sequence(1)
            bert.break_Sequence(1)
            bert.break_Sequence(1)  # PRBS7 Pattern
    if PT == 'PLTPAT':
        bert.restart_All()
        bert.break_Sequence(1)
        bert.break_Sequence(1)
...
def CDR_cap_array_test(VCO_trim):
    ...
    aardvark.basic_write(0x48, [0x0B, 0x33])
    aardvark.basic_write(0x4D, [0x20, 0x05])
    aardvark.basic_write(0x52, [0x0B, 0x33])
...
def CDR_chrg_pmp_test(chrg_pmp=0xCF):
...
SIM_FILE_LIST_TP3 = ["3m_3_S",
                     "CIC_rev0p6"]  # ["DoNothing","1m_Cable","4p5m_Cable","CIC_rev0p6"] ["0p5m_2_S","1p3m_1_S","2m_5_S","3m_3_S","CIC_rev0p6"]
JBERT_VS = [800]
V12 = [1.2, 1.26, 1.14]  # [1.2,1.26,1.14]
SWIFT_RXEQ = ["ENABLED"]
JBERT_ISI = [-6, 0]  # [-9,-6,-3,0]
LinkRa = [0x14, 0xA]  # [0x14,0xA,0x6]
VoltageSwing_PreEmphasis = [[0, 0], [1, 1]]
STAGE = ["TP2"]  # ["TP2","TP3","TP3_EQ"]
...
current = 0
if CHIP == 'SWIFT':
    MODE = ["ReTimerMode"]  # ["ReTimerMode","ReDriverMode"] SWIFT
elif CHIP == 'PARADE':
    MODE = ["ReTimerMode_noSSC", "ReTimerMode_SSC", "ReDriverMode"]  # PARADE

VCO_trim = [0x7FC, 0x7FD, 0x7FF]
rz_sel = [0x24, 0x14]
chrg_pmp = [0xCF, 0xDF, 0xEF, 0xFF]
###############################################
###################AUTOMATION##################
###############################################
...
result_all = [
    ["Part", "LR", "LC", "Temperature", "Cable", "JBERT", "JBERT_ISI", "V12", "SWIFT_RXEQ", "VSL_PE", "tj", "rj", "dj",
     "eye H", "eye W", "eye W UI", "Current", "vpp", "vamp", "Mode", "Stage", "VCO_CAL", "rz_sel", "chrg_pmp"]]
part = 103  # 'p01' for PARADE
print(
    "#################  NOW I'M TESTING %s PART # %s RX %s %s #################" % (CHIP, part, RX_PORT, LaneCount(LC)))
###############################################
    ...
    for LR in LinkRa:
        for mode in MODE:
            ...
            for vco in VCO_trim:
                for rz in rz_sel:
                    for pmp in chrg_pmp:
                        CDR_cap_array_test(vco)
                        CDR_rz_sel([rz])
                        CDR_chrg_pmp_test([pmp])
                        print("VCO_trim = %s rz_sel = %s chrg_pmp = %s" % (hex(vco), hex(rz), hex(pmp)))

                        LinkTraining_DPT200toDPR100(0x14, 0x2, 0, 3)  # Establish LT to config CTLE on scope
                        if part in (102, 101):
                            aardvark.execute_batch_file(r"C:\Caesal\Bootup_Cali\CDR0_CDR1_SEQ_RC_OSC_CAL_101_102.xml")
                            time.sleep(2)
                            print("#################  Special WA for 101 & 102  #################")
                        ...
                        for vs_in in JBERT_VS:
                            ...
                            JBERT_Rate_Pattern(LR, LC)  # JBERT Set LinkRate on L0L1 and Pattern
                            bert.OutputOff(1)
                            time.sleep(1)
                            bert.OutputOn(1)
                            for stage in STAGE:
                                ...
                                for sim_file in SIM_FILE_LIST:  # Select Cable model
                                    ...
                                    for isi in JBERT_ISI:
                                        ...
                                        if LC == 0x2:  # change L1 ISL value from JBERT
                                            bert.set_Isi(1, 2, isi_i)  # Insertion loss from JBERT
                                        for v12 in V12:
                                            pwr.set_VoltageP25V(v12)  # SET VOLTAGE TOLERANCE RANGE
                                            current = pwr.get_CurrP25V()
                                            print("Power Supply set to V%s" % v12)
                                            for EQ in SWIFT_RXEQ:
                                                for VSPE in VoltageSwing_PreEmphasis:
                                                    VS = VSPE[0]
                                                    PE = VSPE[1]
                                                    LinkTraining_DPT200toDPR100(LR, 0x2, VS,
                                                                                PE)  # Always 2L LT when testing L0 or L1
                                                    #                                        if stage == 'TP3_EQ':
                                                    #                                            AUTOSCALE_CTLE()               # AUTOSCALE EQ WAVEFORM for p2p measurement
                                                    ...
                                                    result = [part, LinkRate(LR), LaneCount(LC), target_temp, sim_file,
                                                              ...
                                                              eyeW_UI, current, vpp, vamp, mode, stage, hex(vco),
                                                              hex(rz), hex(pmp)]
                                                    ...
                                                    if LR == 0xA:
                                                        img_name = DIR_SAVE + r"\part%s\%s_%s_part%s_V%s_RX%s_PRBS7_%s_%s_%smV_Cable%s_ISI%sdB_EQ%s_VS%d_PE%d_%dC_%s_%s_%s_%s_%s" % (
                                                            part, CHIP, stage, part, PowerSupply(v12), RX_PORT,
                                                            LinkRate(LR), LaneCount(LC), vs_in, sim_file, isi, EQ, VS,
                                                            PE, target_temp, mode, hex(vco), hex(rz), hex(pmp),
                                                            today_datetime)
                                                    if LR == 0x14:
                                                        img_name = DIR_SAVE + r"\part%s\%s_%s_part%s_V%s_RX%s_CP2520_%s_%s_%smV_Cable%s_ISI%sdB_EQ%s_VS%d_PE%d_%dC_%s_%s_%s_%s_%s" % (
                                                            part, CHIP, stage, part, PowerSupply(v12), RX_PORT,
                                                            LinkRate(LR), LaneCount(LC), vs_in, sim_file, isi, EQ, VS,
                                                            PE, target_temp, mode, hex(vco), hex(rz), hex(pmp),
                                                            today_datetime)
...
today_datetime = datetime.today().strftime('%m%d%Y_%H%M%S')
filename_o = r"C:\Caesal\test\%s_part%s_RX%s_%s_TP2_HBRHBR2_RT_VCOCAL_%s.xlsx" % (CHIP, part, RX_PORT, LaneCount(LC),
                                                                                  today_datetime)
